...
export const EventList = () => {
    ...
    const getFriendString = () =>{
        return setFriendstring(friends.map(friend => { return `userId=${friend}` }).join("&"))
    }
    useEffect(() => {
        getFriendsOfUser()
    }, [])
    ...
    const getEvents = () => {
        ...
        return getEventsForUser(friendString).then(eventsFromApi => { setEvent(getSoonestEvent(eventsFromApi)) })
    }
